...
struct TimeGrid
    ...
    TimeGrid(timestart::DateTime, fs::Float64, offset::Int = 0) =
...
#=
mutable struct TimeSeries{T, N} <: AbstractArray{T, N}
    data::Array{T, N}
    tg::TimeGrid
end

mutable struct TimeEvents{T, N} <: AbstractVector{T}
    data::Vector{UnitRange}
    tg::TimeGrid
end

mutable struct TimeSegments{T} <: AbstractVector{UnitRange{T}}
    #data::Vector{URange{T}}
    from::Vectot{T}
    to::Vector{T}
    # ??? types ???
    tg::TimeGrid
...
# tg1 = TimeGrid(Dates.now(),250.0)
#
# tg2 = TimeGrid(Dates.now(),100.0)
#
# toindex = index_transform(tg1, tg2)
#
# Juno.@enter toindex(100)
#
# sig1.tg
# sig2.tg
# toindex2 = index_transform()
#
# for i = eachindex(sig1)
#     sig1[i] - sig2[toindex2(i)]
# end
